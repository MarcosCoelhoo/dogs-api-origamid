<?php 
  ...
  function api_commment_post($request) {
    $user = wp_get_current_user();
    $user_id = $user->ID;

  
    if ($user_id === 0) {
      $response = new WP_Error('error', 'usuário sem permissao', ['status' => 401]);
  ...
  function register_api_commment_post() {
    register_rest_route('api', '/comment/(?P<id>[0-9]+)', [
      'methods' => WP_REST_Server::CREATABLE,
      'callback' => 'api_commment_post',
    ]);
  };

  add_action('rest_api_init', 'register_api_commment_post');  
