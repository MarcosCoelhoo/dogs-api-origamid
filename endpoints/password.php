...
// PERDEU A SENHA 
  ...
  function api_password_lost($request) {
    ...
    if (empty($login)) {
      $response = new WP_Error('error', 'Informe o email or login', ['status' => 406]);
    ...
    if (empty($user)) {
      $response = new WP_Error('error', 'O usúario informado não existe no nosso banco de dados, tente outro por favor man. (salve pro marcos do futuro✌🏻)', ['status' => 401]);
    ...
    $message = "Utilize o link abaixo se quiser ser salvo da bomba nuclear que irá estourar após o dia 2 de janeiro de 2038 (salve markonha ✌🏻✌🏻✌🏻😴) \r\n";
  ...
  function api_password_reset($request) {
    ...
    if (empty($user)) {
      $response = new WP_Error('error', 'O usuário não existe', ['status' => 401]);
      return rest_ensure_response($response);
    }
